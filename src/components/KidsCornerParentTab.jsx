...
import { Star, Timer, Repeat, Plus, Circle, Trash2, CheckCircle2, Gift } from 'lucide-react';
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <section className={cardBaseClasses + " p-8"}>
                ...
                <form onSubmit={handleAddKidItem} className="flex flex-wrap items-stretch gap-3 mb-6 w-full">
                    <input type="text" value={newItem} onChange={e=>setNewItem(e.target.value)} placeholder={`Add new ${activeList}...`} className={inputBaseClasses + " flex-1 min-w-[180px]"} />
                    
                    {activeList === 'tasks' && (
                        <div className={inputBaseClasses + " flex items-center gap-2 px-3 w-28 shrink-0 !py-0"}>
                            <Timer className="w-4 h-4 text-slate-400 shrink-0"/>
                            <input type="number" value={newTimeLimit} onChange={e=>setNewTimeLimit(e.target.value)} placeholder="Mins" className="w-full bg-transparent outline-none font-bold placeholder:text-slate-400 py-3 text-sm" />
                        </div>
                    )}

                    {activeList === 'rewards' && (
                        <>
                            <div className={inputBaseClasses + " flex items-center gap-2 px-3 w-24 shrink-0 !py-0"}>
                                <Star className="w-4 h-4 text-amber-500 shrink-0"/>
                                <input type="number" value={newRewardCost} onChange={e=>setNewRewardCost(e.target.value)} placeholder="Cost" className="w-full bg-transparent outline-none font-bold placeholder:text-slate-400 py-3 text-sm" />
                            </div>
                            <button 
                                type="button" 
                                onClick={() => setIsPermanentReward(!isPermanentReward)}
                                className={`px-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all flex flex-col items-center justify-center shrink-0 shadow-sm ${isPermanentReward ? 'bg-indigo-100 border-indigo-300 text-indigo-700 dark:bg-indigo-900/50 dark:border-indigo-700 dark:text-indigo-300 ring-2 ring-indigo-500/50' : 'bg-slate-50 border-slate-200 text-slate-400 dark:bg-slate-950 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900'}`}
                    ...
                    <button type="submit" className="bg-gradient-to-r from-amber-400 to-amber-500 text-white px-5 rounded-xl font-bold shadow-md shadow-amber-500/20 hover:scale-105 active:scale-95 transition-all shrink-0 flex items-center justify-center"><Plus className="w-5 h-5"/></button>
                </form>
                    ...
                    {activeList === 'tasks' ? (
                        <>
                            {tasks.filter(t => t.assignee === 'Leo' && !t.completed).map(task => (
                                <div key={task.id} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 group transition-colors hover:border-amber-200 dark:hover:border-amber-800/50">
                                    <button onClick={() => toggleTask(task, userProfile)} className="transform transition-transform active:scale-75 shrink-0"><Circle className="w-6 h-6 text-slate-300 hover:text-amber-500" /></button>
                                    ...
                                    <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tasks', task.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 shrink-0"><Trash2 className="w-5 h-5"/></button>
                                </div>
                            ))}
                            {tasks.filter(t => t.assignee === 'Leo' && t.completed).map(task => (
                                <div key={task.id} className="flex items-center gap-4 p-4 rounded-2xl opacity-50 grayscale group">
                                    <button onClick={() => toggleTask(task, userProfile)} className="shrink-0"><CheckCircle2 className="w-6 h-6 text-emerald-500" /></button>
                                    <span className="flex-1 font-bold line-through text-slate-500 truncate">{task.title}</span>
                                    <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tasks', task.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 p-2 shrink-0"><Trash2 className="w-5 h-5"/></button>
                                </div>
                            ))}
                        </>
                    ) : activeList === 'rewards' ? (
                        <>
                            {leoData.rewards?.filter(r => r.status === 'pending').map(reward => (
                                <div key={reward.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl border border-rose-200 dark:border-rose-800/60 bg-gradient-to-br from-rose-50 to-white dark:from-rose-900/10 dark:to-slate-900 shadow-sm animate-in slide-in-from-left-4">
                                    <div className="flex items-center gap-3.5">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[10px] uppercase font-black tracking-widest text-rose-500 bg-rose-100/50 dark:bg-rose-900/40 px-2 py-0.5 rounded shadow-sm mt-1 inline-block">Pending Approval</span>
                                        </div>
                                    </div>
                                    <button onClick={() => approveReward(reward.id)} className="w-full sm:w-auto bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md shadow-emerald-500/20 active:scale-95 transition-all shrink-0">Approve Request</button>
                                </div>
                            ))}
                        ...
                        <>
                            {leoData[activeList]?.filter(i=>!i.completed).map(item => (
                                <div key={item.id} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 group hover:border-indigo-200 dark:hover:border-indigo-800/50 transition-colors">
                                    <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'leodata', item.id), {completed: true})} className="transform transition-transform active:scale-75 shrink-0"><Circle className="w-6 h-6 text-slate-300 hover:text-indigo-500" /></button>
                                    <span className="flex-1 font-bold text-slate-800 dark:text-slate-200 truncate">{item.title}</span>
                                    <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'leodata', item.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 shrink-0"><Trash2 className="w-5 h-5"/></button>
                                </div>
                            ))}
                            {leoData[activeList]?.filter(i=>i.completed).map(item => (
                                <div key={item.id} className="flex items-center gap-4 p-4 rounded-2xl opacity-50 grayscale group">
                                    <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'leodata', item.id), {completed: false})} className="shrink-0"><CheckCircle2 className="w-6 h-6 text-emerald-500" /></button>
                                    <span className="flex-1 font-bold line-through text-slate-500 truncate">{item.title}</span>
                                    <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'leodata', item.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 p-2 shrink-0"><Trash2 className="w-5 h-5"/></button>
                                </div>
                            ))}
                        </>
                    )}
