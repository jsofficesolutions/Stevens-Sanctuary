        ...
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className={cardBaseClasses + " flex flex-col lg:h-[700px]"}>
                    ...
                    <form onSubmit={handleAddShopping} className="flex gap-3 mb-6 w-full max-w-full">
                        <input type="text" value={newItem} onChange={e=>setNewItem(e.target.value)} placeholder="Add item..." className={inputBaseClasses + " flex-1 min-w-0"} />
                        <button type="submit" className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-6 rounded-xl font-bold shadow-md shadow-emerald-500/20 hover:scale-105 active:scale-95 transition-all shrink-0 flex items-center justify-center"><Plus className="w-6 h-6"/></button>
                    </form>
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                        {shoppingList.filter(i=>!i.completed).map(item => (
                            <div key={item.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 group transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-800">
                                <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shopping', item.id), {completed:true})} className="transform transition-transform active:scale-75 shrink-0"><Circle className="w-6 h-6 text-slate-300 hover:text-emerald-500" /></button>
                                <span className="flex-1 font-bold text-slate-800 dark:text-slate-200 truncate">{item.title}</span>
                                <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shopping', item.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 shrink-0"><Trash2 className="w-4 h-4"/></button>
                            </div>
                        ))}
                            ...
                            <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
                                ...
                                {shoppingList.filter(i=>i.completed).map(item => (
                                    <div key={item.id} className="flex items-center gap-4 p-3 rounded-xl opacity-50 grayscale hover:grayscale-0 transition-all group">
                                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shopping', item.id), {completed:false})} className="shrink-0"><CheckCircle2 className="w-6 h-6 text-emerald-500" /></button>
                                        <span className="flex-1 font-bold line-through text-slate-500 truncate">{item.title}</span>
                                        <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shopping', item.id))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 shrink-0"><Trash2 className="w-4 h-4"/></button>
                                    </div>
                                ))}
                            </div>
...
export function FinanceTab({ finances, db, appId, user }) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500 w-full max-w-full overflow-hidden">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start w-full max-w-full">
                <div className="space-y-8 w-full max-w-full">
                    ...
                    <div className={cardBaseClasses + " w-full max-w-full"}>
                        ...
                        <form onSubmit={addIncome} className="flex gap-2 mb-6 w-full max-w-full items-center">
                            <input type="text" value={newIncomeName} onChange={e=>setNewIncomeName(e.target.value)} placeholder="Source name (e.g., Salary)..." className={inputBaseClasses + " flex-1 min-w-0"} required />
                            <input type="number" value={newIncomeAmt} onChange={e=>setNewIncomeAmt(e.target.value)} placeholder="£ Amount" className={inputBaseClasses + " w-24 shrink-0"} required />
                            <button type="submit" className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 h-12 rounded-xl font-bold flex items-center justify-center transition-all active:scale-95 shadow-md shadow-emerald-500/20 shrink-0">
                        ...
                        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                            {(currentFinance.incomes||[]).map(inc => (
                                <div key={inc.id} className="flex justify-between items-center p-4 bg-slate-50/50 dark:bg-slate-950/50 rounded-2xl border border-slate-100 dark:border-slate-800/50 group transition-colors hover:border-emerald-200 dark:hover:border-emerald-800/50">
                                    <span className="font-bold text-slate-800 dark:text-slate-200 truncate pr-2">{inc.name}</span>
                    ...
                    <div className={cardBaseClasses + " w-full max-w-full"}>
                        ...
                        <form onSubmit={addOutgoing} className="flex flex-col gap-3 mb-6 bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-200/60 dark:border-slate-800 w-full max-w-full">
                            <div className="flex gap-2 items-center w-full">
                                <input type="text" value={newOutName} onChange={e=>setNewOutName(e.target.value)} placeholder="Bill / Expense..." className="flex-1 bg-transparent outline-none text-sm font-bold px-2 placeholder:text-slate-400 min-w-0" required />
                                <input type="number" value={newOutAmt} onChange={e=>setNewOutAmt(e.target.value)} placeholder="£" className="w-20 bg-transparent outline-none text-sm font-bold border-l border-slate-200 dark:border-slate-700 pl-3 placeholder:text-slate-400 shrink-0" required />
                            </div>
                            <div className="flex flex-wrap gap-2 items-center justify-between mt-1 pt-3 border-t border-slate-200/60 dark:border-slate-800 w-full">
                                ...
                                <button type="submit" className="bg-rose-500 hover:bg-rose-600 text-white px-5 py-2 rounded-xl font-bold text-sm shadow-sm active:scale-95 transition-transform flex items-center justify-center gap-1"><Plus className="w-4 h-4"/> Add</button>
                            </div>
                        </form>
                        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                            {(currentFinance.outgoings||[]).map(out => (
                                <div key={out.id} className="flex justify-between items-center p-4 bg-slate-50/50 dark:bg-slate-950/50 rounded-2xl border border-slate-100 dark:border-slate-800/50 group transition-colors hover:border-rose-200 dark:hover:border-rose-800/50">
                                    <div className="flex flex-col gap-1 overflow-hidden pr-2">
                                        <span className="font-bold text-slate-800 dark:text-slate-200 truncate">{out.name}</span>
                ...
                {/* BUDGET POTS */}
                <div className={cardBaseClasses + " w-full max-w-full"}>
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-8 gap-4 w-full">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-950 px-5 py-3 rounded-2xl border border-slate-200/60 dark:border-slate-800 text-right w-full sm:w-auto shrink-0">
                    ...
                    <form onSubmit={addPot} className="flex gap-2 mb-8 w-full max-w-full items-center">
                        <input type="text" value={newPotName} onChange={e=>setNewPotName(e.target.value)} placeholder="Pot name (e.g., Groceries)..." className={inputBaseClasses + " flex-1 min-w-0"} required />
                        <input type="number" value={newPotAlloc} onChange={e=>setNewPotAlloc(e.target.value)} placeholder="£ Alloc" className={inputBaseClasses + " w-24 shrink-0"} required />
                        <button type="submit" className="bg-violet-500 hover:bg-violet-600 text-white px-4 h-12 rounded-xl font-bold flex items-center justify-center transition-all active:scale-95 shadow-md shadow-violet-500/20 shrink-0">
                            <Plus className="w-5 h-5"/>
                        </button>
                    </form>
                    ...
                    <div className="space-y-5 max-h-[660px] overflow-y-auto pr-1 custom-scrollbar">
                        {(currentFinance.pots||[]).map(pot => {
                            ...
                            return (
                                <div key={pot.id} className="flex flex-col p-5 rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm gap-4 group hover:shadow transition-shadow w-full max-w-full">
                                    <div className="flex items-center justify-between w-full">
                                        <span className="font-black text-lg text-slate-800 dark:text-slate-100 tracking-tight truncate pr-2">{pot.name}</span>
                                        <button type="button" onClick={()=>removeItem('pots', pot.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1 shrink-0"><Trash2 className="w-4 h-4"/></button>
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between text-sm bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-200/50 dark:border-slate-800/50 gap-3 w-full">
                                        <div className="flex items-center gap-5 justify-between sm:justify-start w-full sm:w-auto">
                                            ...
                                            <div className="flex flex-col border-l border-slate-200 dark:border-slate-800 pl-5">
                                                ...
                                                <div className="flex items-center gap-0.5">
                                                    ...
                                                    <input type="number" value={pot.spent} onChange={e=>updatePotSpent(pot.id, e.target.value)} className="w-14 bg-transparent outline-none font-bold text-rose-500 focus:ring-1 focus:ring-rose-400 rounded px-0.5" />
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex flex-col text-left sm:text-right border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-200/60 dark:border-slate-800 w-full sm:w-auto">
